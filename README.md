...
| 機能 | 詳細 |
|---|---|
| **5 コース対応** | 東山 / 北山東部 / 北山西部 / 西山 / 京北 の全 5 コースを実装済み（Phase 8 で完成） |
| **実 OSM ルート + GSI 標高 + ibuki CC0 GPX** | OpenStreetMap (ODbL) の hiking relation と国土地理院 getelevation API を統合。北山西部の OSM 未マップ区間 (marker 70-90) と京北コースは [ibuki.run](https://ibuki.run/) の CC0 GPS 軌跡で補完 |
...
# ibuki CC0 由来コース (京北、北山西部 marker 70-90 ギャップ補完)
node tools/build_courses_ibuki.js   # ibuki-*.gpx → data/courses/{keihoku,kitayama-west}.json
```

OSM データが更新された場合のみ実行。標高 API 呼び出しが各コース 150〜220 ポイント × 200ms 待機なのでコース 1 本あたり約 30〜45 秒。
...
- [x] **Phase 8**: 京北コース実装 + 北山西部 marker 70-90 ギャップ補完（ibuki.run の CC0 GPX を統合）

## 既知の制約

- **OS バックグラウンド GPS**: ブラウザ仕様で不可。Wake Lock + 経路追従補間で代替
- **京北コースのチェックポイント精度**: ibuki GPX (CC0) のみが入手可能なソースで、起点・終点・最高点 (栃本峠周辺) 以外は距離マイルストーンによる仮置き。実標識との対応はユーザの実踏査または公式紙地図で確認のこと
- **本線距離 84km vs 90km**: 京都市公式の「本線約 84km」は概算値。本アプリは OSM hiking relation + ibuki 実走 GPS から計測した約 90km を実距離として表示する（東山が公称より長く、北山西部が ibuki 補完で公称 19.5km → 実 21km）
